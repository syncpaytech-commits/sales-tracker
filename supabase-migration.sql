-- Supabase Migration Script for Sales Tracker
-- This creates all tables in PostgreSQL (Supabase) format
...
-- Opportunities (must be created before leads due to foreign key)
...
ALTER TABLE opportunities ADD CONSTRAINT fk_opportunities_leadId FOREIGN KEY ("leadId") REFERENCES leads(id) ON DELETE SET NULL;
...
CREATE TABLE "auditLogs" (
  ...
  "deletedBy" INTEGER NOT NULL REFERENCES users(id),
...
CREATE POLICY "Users can view their own leads" ON leads
  FOR SELECT
  USING (
    "ownerId" = (SELECT id FROM users WHERE users.id = auth.uid()::int)
    OR
...
CREATE POLICY "Users can insert their own leads" ON leads
  FOR INSERT
  WITH CHECK ("ownerId" = (SELECT id FROM users WHERE users.id = auth.uid()::int));

CREATE POLICY "Users can update their own leads" ON leads
  FOR UPDATE
  USING ("ownerId" = (SELECT id FROM users WHERE users.id = auth.uid()::int));
...
CREATE POLICY "Users can view their own call logs" ON "callLogs"
  FOR SELECT
  USING (
    "agentId" = (SELECT id FROM users WHERE users.id = auth.uid()::int)
    OR
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid()::int AND users.role = 'admin')
  );
...
CREATE POLICY "Users can insert their own call logs" ON "callLogs"
  FOR INSERT
  WITH CHECK ("agentId" = (SELECT id FROM users WHERE users.id = auth.uid()::int));

-- RLS Policies for notes (based on creator)
CREATE POLICY "Users can view their own notes" ON notes
  FOR SELECT
  USING (
    "createdBy" = (SELECT id FROM users WHERE users.id = auth.uid()::int)
    OR
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid()::int AND users.role = 'admin')
  );
...
CREATE POLICY "Users can insert their own notes" ON notes
  FOR INSERT
  WITH CHECK ("createdBy" = (SELECT id FROM users WHERE users.id = auth.uid()::int));
